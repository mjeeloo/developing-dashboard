...
function App() {
  ...
  const assignedTasks = useMemo(
    () =>
      tasks.filter((task) => {
        ...
        return Array.isArray(task.assignees) && task.assignees.length > 0 && statusType !== 'closed';
      }),
    [tasks],
  );
  ...
  const tasksByAssignee = useMemo(() => {
    return assignedTasks.reduce((accumulator, task) => {
      const owners = Array.isArray(task.assignees) ? [...new Set(task.assignees)] : [];
                ...
                <tbody>
                  {supportTasks.map((task) => (
                    <tr key={task.id}>
                      ...
                      <td>{task.assigneeLabel ?? 'Unassigned'}</td>
                      <td>
